...
interface NavItem {
  ...
  tone?: string;
  badgeKey?:
    "announcements" | "events" | "notifications" | "pendingLeave" | "messages";
...
> = {
  event: {
    label: "Event",
    icon: Calendar,
    tone: "text-emerald-300 bg-emerald-500/14",
  },
  song: { label: "Song", icon: Music2, tone: "text-sky-300 bg-sky-500/14" },
  setlist: {
    label: "Set",
    icon: ListChecks,
    tone: "text-violet-300 bg-violet-500/14",
  },
  person: {
    label: "Person",
    icon: User,
    tone: "text-amber-300 bg-amber-500/14",
  },
...
function MobileBadge({
  ...
  return (
    <span
      className={`absolute -top-0.5 -right-0.5 flex items-center justify-center min-w-[15px] h-[15px] px-1 rounded-full ${bg} text-white text-[9px] font-bold leading-none shadow-sm`}
    >
      {count > 9 ? "9+" : count}
    </span>
  );
...
function SidebarBadge({
  ...
  return (
    <span
      className={`flex items-center justify-center h-5 min-w-[20px] px-1.5 rounded-full ${bg} text-white text-[10px] font-bold leading-none`}
    >
      {count > 9 ? "9+" : count}
    </span>
  );
...
export function Navigation({
  ...
  const [switchingAccountMeta, setSwitchingAccountMeta] = useState<{
    id: string;
    name: string;
    email: string;
  } | null>(null);
  ...
  const isActive = useCallback(
    (item: NavItem) => {
      ...
      if (
        item.path === "/leadership/overview" ||
        item.path === "/leadership/church"
      ) {
  ...
  useEffect(() => {
    const handleStyleChange = () =>
      setMobileNavStyle(getStoredMobileNavStyle());
  ...
  useEffect(() => {
    ...
    const target =
      drawerPanel === "settings"
        ? mobileSettingsScrollRef.current
        : mobileMenuScrollRef.current;
    const savedTop =
      drawerPanel === "settings"
        ? mobileSettingsScrollTopRef.current
  ...
  useEffect(() => {
      ...
      setGlobalSearchResults([
        {
          ...
          kind: "page",
          title: "Events",
          subtitle: "Schedule, create, and review services",
          path: "/events",
        },
        {
          id: "page-songs",
          kind: "page",
          title: "Songs",
          subtitle: "Chord charts and song library",
          path: "/songs",
        },
        {
          id: "page-sets",
          kind: "page",
          title: "Sets",
          subtitle: "Setlists and past worship sets",
          path: "/sets",
        },
        {
          id: "page-team",
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        const [eventsRes, songsRes, profilesRes, setlistsRes] =
          await Promise.all([
            supabase
              .from("events")
            ...
            supabase
              ...
              .or(
                `title.ilike.${pattern},artist.ilike.${pattern},song_key.ilike.${pattern}`,
              )
              .order("title")
              .limit(6),
            supabase
        ...
        const eventResults: GlobalSearchResult[] = (
          ...
          title: event.title,
          subtitle:
            [event.event_type, event.event_date].filter(Boolean).join(" · ") ||
            "Event",
          path: `/events/${event.id}`,
        }));

        const songResults: GlobalSearchResult[] = (
          (songsRes.data || []) as Array<{
            id: string;
            title: string;
            artist?: string | null;
          ...
          title: song.title,
          subtitle:
            [song.artist, song.song_key ? `Key ${song.song_key}` : ""]
              .filter(Boolean)
              .join(" · ") || "Song library",
          path: `/songs?search=${encodeURIComponent(song.title)}`,
        }));

        const personResults: GlobalSearchResult[] = (
        ...
        const setlistResults: GlobalSearchResult[] = (
          (setlistsRes.data || []) as Array<{
            id: string;
            status?: string | null;
          ...
          .map((setlist) => {
            const event = Array.isArray(setlist.events)
              ? setlist.events[0]
              : setlist.events;
            return { setlist, event };
          })
          .filter(({ event }) => {
            const haystack = [event?.title, event?.event_date, "setlist", "set"]
              .filter(Boolean)
              .join(" ")
              .toLowerCase();
          ...
          .map(({ setlist, event }) => ({
            ...
            subtitle:
              [setlist.status, event?.event_date].filter(Boolean).join(" · ") ||
              "Setlist",
  ...
  const handleMobileNavStyleChange = async (style: MobileNavStyle) => {
    ...
    try {
      ...
      toast(
        "success",
        `Navigation style set to ${style === "floating" ? "Floating" : "Docked"}`,
      );
    } catch {
  ...
  const handleSwitchAccount = async (targetUserId: string) => {
    ...
    const targetAccount = savedAccounts.find(
      (account) => account.userId === targetUserId,
    );
    if (!targetAccount) {
  ...
  const handleSaveAnotherAccount = async (e: React.FormEvent) => {
    ...
    const { error } = await addSavedAccount(
      addAccountEmail,
      addAccountPassword,
    );
  ...
  const baseProfileMenuItems: Array<{
    ...
    },
    {
      icon: Video,
      label: "Videos",
      desc: "Training and reference videos",
      path: "/videos",
      show: true,
      color: "#0ea5e9",
    },
  ...
  const renderMobileSettingsPanel = () => (
    <>
      <div
        className="border-b border-black/[0.06] px-4 pb-4 dark:border-white/[0.08]"
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[20px] font-black leading-tight text-gray-900 dark:text-white">
              Settings
            </p>
      ...
      <div
        ...
        onScroll={(event) => {
          mobileSettingsScrollTopRef.current = event.currentTarget.scrollTop;
        }}
        ...
        <div className="space-y-4">
          <section className="rounded-[1.55rem] border border-black/[0.06] bg-black/[0.02] p-3.5 dark:border-white/[0.08] dark:bg-white/[0.035]">
            <div className="mb-3">
              ...
              <h3 className="mt-1 text-[15px] font-black tracking-[-0.02em] text-gray-900 dark:text-white">
                Choose bottom nav style.
              </h3>
            </div>
            ...
            <p className="mt-2 text-[10px] font-semibold text-gray-500 dark:text-gray-400">
              ...
              {getDefaultMobileNavStyle() === "floating"
                ? "Floating"
                : "Docked"}
            </p>
          </section>
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-[13px] font-black text-gray-900 dark:text-white">
                        {account.displayName || "Saved account"}
                      </p>
                      <p className="truncate text-[10px] font-mono font-semibold text-gray-500 dark:text-gray-400">
                        {account.email}
                      </p>
                    </div>
                    {isCurrent ? (
    ...
    <div className="dark contents">
              ...
              <div className="border-b border-white/[0.08] p-3">
                <div className="flex items-center gap-3 rounded-[0.75rem] bg-white/[0.045] p-2.5">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-0.5 truncate text-[11px] font-semibold leading-tight text-white/42">
                      {profile?.email}
                    </p>
                  </div>
                </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="mt-0.5 truncate text-[10px] font-semibold leading-tight text-white/38">
                              {account.email}
                            </p>
                          </div>
                          {isCurrent ? (
                ...
                {!globalSearchLoading && globalSearchResults.length === 0 ? (
                  <div className="px-4 py-8 text-center">
                    ...
                    <p className="mt-1 text-[12px] font-semibold text-white/42">
                      Try a song title, service name, setlist, or team member.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-1">
                    {globalSearchResults.map((result) => {
                      ...
                      return (
                        ...
                        >
                          <span
                            className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-[0.65rem] ${meta.tone}`}
                          >
      ...
      <div
        ...
        style={{
          ...
          boxShadow:
            "0 14px 34px -30px rgba(0,0,0,0.85), inset 0 -1px 0 rgba(255,255,255,0.06)",
          top: "-1px",
            ...
            >
              <div className="flex h-full min-h-0 flex-col">
                {drawerPanel === "settings" ? (
                  ...
                  <>
                    <div
                      className="border-b border-black/[0.06] px-5 pb-5 dark:border-white/[0.08]"
                      style={{
                        paddingTop: "calc(env(safe-area-inset-top) + 1.6rem)",
                      }}
                    >
                      <div className="flex items-center gap-4">
                        ...
                        />
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-[21px] font-black leading-tight text-gray-900 dark:text-white">
                            {displayName || fullName || "Profile"}
                          </p>
                          <button
                            onClick={() => handleNav("/profile")}
      ...
      {!hideMobile && !hideMobileAll && (
        <div
          ...
          style={{
            paddingBottom: useDockedMobileNav
              ? "0px"
          ...
          >
            ...
            >
              ...
              {mobileNavItems.map((item) => {
                ...
                const Icon = item.icon;
                const badge = getBadgeCount(item);
                const navItemColor = active
                  ? "var(--mobile-nav-active-color)"
                  : "var(--mobile-nav-inactive-color)";
                return (
                  <button
                    key={item.path}
                    ...
                    {active && !useDockedMobileNav && (
                      <motion.div
                        ...
                        style={{
                          ...
                          boxShadow:
                            "0 1px 2px rgba(0,0,0,0.06), 0 0 0 1px rgba(0,0,0,0.02)",
                        }}
                        transition={{
                          type: "spring",
                          stiffness: 500,
      ...
      <Modal
        ...
        onClose={() => {
          if (!addAccountLoading) setShowAddAccountModal(false);
        }}
        title="Save Account"
        size="md"
        ...
        <div className="space-y-5">
          <div>
            ...
            <p className="mt-2 max-w-sm text-[13px] leading-relaxed text-gray-500 dark:text-gray-300">
              Enter the second account once. It will be saved on this device so
              you can switch from the avatar menu anytime.
            </p>
          </div>
          ...
          <form onSubmit={handleSaveAnotherAccount} className="space-y-4">
            <div className="rounded-2xl border border-emerald-200/80 bg-emerald-50/70 px-3.5 py-3 dark:border-emerald-500/20 dark:bg-emerald-500/[0.08]">
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                  <p className="text-[12px] font-black text-emerald-900 dark:text-emerald-200">
                    Your current account stays active.
                  </p>
                  <p className="mt-1 text-[11px] leading-relaxed text-emerald-800/75 dark:text-emerald-200/75">
                    This only stores the other account on this device. It does
                    not log you out of the one you are using now.
                  </p>
                </div>
              </div>
            </div>
          ...
          >
            <div className="relative w-full max-w-sm">
              <motion.div
                ...
                animate={{
                  scale: [0.86, 1.16, 0.98],
                  opacity: [0.18, 0.5, 0.26],
                }}
                transition={{
                  duration: 3.1,
                  repeat: Infinity,
              ...
              <div className="relative mx-auto flex h-32 w-32 items-center justify-center">
                ...
                <motion.div
                  ...
                  animate={{
                    y: [0, -7, 0],
                    rotate: [0, -1.5, 1.5, 0],
                    borderRadius: ["2rem", "2.35rem", "2rem"],
                  }}
                  transition={{
                    duration: 3.1,
                    repeat: Infinity,
