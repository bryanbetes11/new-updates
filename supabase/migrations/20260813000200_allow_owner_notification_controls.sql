-- Allow the approved owner accounts to manage notification controls for their
-- own organization, alongside organization admins.

drop policy if exists "Org admins can update notification settings"
  on public.notification_system_settings;
create policy "Org admins and approved owners can update notification settings"
...
create policy "Org admins and approved owners can update notification rules"
...
begin
  ...
  if v_org_id is null then
    raise exception 'Only organization admins and approved owners can view push readiness'
      using errcode = '42501';
  end if;
