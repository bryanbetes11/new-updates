-- Auto-approval is a creation rule. It must not override a later withdrawal
-- or administrator decision on an existing leave request.
...
begin
  ...
  if coalesce(new.request_type, 'leave') = 'leave' then
    ...
    if tg_op = 'INSERT'
      and not coalesce((v_policy ->> 'approval_required')::boolean, true) then
      new.status := 'approved';
    end if;
