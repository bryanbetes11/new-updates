-- Allow a church admin to grant an organization-wide attendance consideration
-- for one quarter without deleting attendance history. Existing late/absent
-- outcomes and finalized confirmed assignments without a mark become excused.

create or replace function public.reset_team_attendance_for_consideration(
  p_year integer,
  p_quarter integer,
  p_reason text
)
...
begin
  ...
  if v_start_date > timezone('Asia/Manila', now())::date then
    raise exception 'A future attendance quarter cannot be reset';
  end if;
...
revoke all on function public.reset_team_attendance_for_consideration(integer, integer, text)
  from public, anon;
grant execute on function public.reset_team_attendance_for_consideration(integer, integer, text)
  to authenticated;

comment on function public.reset_team_attendance_for_consideration(integer, integer, text) is
  'Admin-only, tenant-scoped quarterly attendance consideration. Preserves history by excusing accountable outcomes instead of deleting records.';
